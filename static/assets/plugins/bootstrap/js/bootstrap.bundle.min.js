...
!(function (t, e) {
  ...
  j.on(
    ...
    g(st)
  const ot = 'collapse'
  const rt = { toggle: !0, parent: null }
  const at = { toggle: 'boolean', parent: '(null|element)' }
  const lt = 'show'
  const ct = 'collapse'
  const ht = 'collapsing'
  const dt = 'collapsed'
  ...
  const bt = 'left'
  const vt = 'auto'
  const yt = [mt, gt, _t, bt]
  const wt = 'start'
  const Et = 'end'
  const At = 'clippingParents'
  const Tt = 'viewport'
  const Ot = 'popper'
  const Ct = 'reference'
  const kt = yt.reduce((t, e) => t.concat([`${e}-${wt}`, `${e}-${Et}`]), [])
  const Lt = []
    .concat(yt, [vt])
    .reduce((t, e) => t.concat([e, `${e}-${wt}`, `${e}-${Et}`]), [])
  const xt = 'beforeRead'
  const Dt = 'read'
  const St = 'afterRead'
  const Nt = 'beforeMain'
  ...
  const Mt = 'write'
  const Ht = 'afterWrite'
  const Bt = [xt, Dt, St, Nt, It, Pt, jt, Mt, Ht]
  function Rt(t) {
  ...
  function Jt(t) {
    return zt(t) && Yt(t).position !== 'fixed' ? t.offsetParent : null
  }
  function te(t) {
    for (var e = Wt(t), i = Jt(t); i && Qt(i) && Yt(i).position === 'static'; )
      i = Jt(i)
    return i &&
      (Rt(i) === 'html' || (Rt(i) === 'body' && Yt(i).position === 'static'))
      ? e
      : i ||
          (function (t) {
            const e =
  ...
  const le = {
    ...
    fn(t) {
      ...
      const a = Ut(i.placement)
      const l = ee(a)
      const c = [bt, _t].indexOf(a) >= 0 ? 'height' : 'width'
      if (o && r) {
  ...
  function de(t) {
    ...
    const p = d.y
    let m = void 0 === p ? 0 : p
    const g = r.hasOwnProperty('x')
    const _ = r.hasOwnProperty('y')
    let b = bt
    let v = mt
    const y = window
    if (c) {
  ...
  function Ae(t, e) {
    ...
    const o = Wt(n)
    const r = s ? [o].concat(o.visualViewport || [], we(n) ? n : []) : n
    const a = e.concat(r)
  ...
  function ke(t, e) {
    ...
    const o = i.boundary
    const r = void 0 === o ? At : o
    const a = i.rootBoundary
    const l = void 0 === a ? Tt : a
    const c = i.elementContext
    const h = void 0 === c ? Ot : c
    const d = i.altBoundary
    const u = void 0 !== d && d
    const f = i.padding
    const p = void 0 === f ? 0 : f
    const m = re(typeof p !== 'number' ? p : ae(p, yt))
    const g = h === Ot ? Ct : Ot
    const _ = t.rects.popper
    const b = t.elements[u ? g : h]
          ...
          ? (function (t) {
              ...
              const i =
                ['absolute', 'fixed'].indexOf(Yt(t).position) >= 0 && zt(t)
                  ...
                  : t
              return $t(i)
                ? e.filter((t) => $t(t) && Xt(t, i) && Rt(t) !== 'body')
                : []
            })(t)
          : [].concat(e)
      const s = [].concat(n, [i])
      const o = s[0]
      const r = s.reduce((e, i) => {
        const n = Oe(t, i)
        return (
    ...
    const y = Vt(t.elements.reference)
    const w = Ce({
      reference: y,
      element: _,
      strategy: 'absolute',
    ...
    const O = t.modifiersData.offset
    if (h === Ot && O) {
      const C = O[s]
      Object.keys(T).forEach((t) => {
        const e = [_t, gt].indexOf(t) >= 0 ? 1 : -1
        const i = [mt, gt].indexOf(t) >= 0 ? 'y' : 'x'
        T[t] += C[i] * e
      })
    }
    return T
  ...
  function Le(t, e) {
    ...
    const h = ce(n)
    const d = h ? (a ? kt : kt.filter((t) => ce(t) === h)) : yt
  ...
  const Ne = {
    ...
    fn(t) {
      ...
      const r = ke(e, { elementContext: 'reference' })
      const a = ke(e, { altBoundary: !0 })
      const l = De(r, n)
      const c = De(a, s, o)
      const h = Se(l)
      const d = Se(c)
    ...
    fn(t) {
      ...
      const r = Lt.reduce(
        (t, i) => (
          (t[i] = (function (t, e, i) {
            const n = Ut(t)
            const s = [bt, mt].indexOf(n) >= 0 ? -1 : 1
            const o = 'function' === typeof i ? i({ ...e, placement: t }) : i
            let r = o[0]
            let a = o[1]
            return (
              (r = r || 0),
  ...
  const je = {
    ...
    fn(t) {
      ...
      const _ = Ut(e.placement)
      const b = ce(e.placement)
      const v = !b
      const y = ee(_)
      const w = y === 'x' ? 'y' : 'x'
      ...
      if (E) {
        if (o || a) {
          ...
          const N = E[y] - g[L]
          const I = f ? -T[x] / 2 : 0
          const P = b === wt ? A[x] : T[x]
          const j = b === wt ? -T[x] : -A[x]
          const M = e.elements.arrow
          const H = f && M ? Kt(M) : { width: 0, height: 0 }
          ...
          const W = B[L]
          const $ = oe(0, A[x], H[x])
          const z = v ? A[x] / 2 - I - $ - R - O : P - $ - R - O
          const q = v ? -A[x] / 2 + I + $ + W + O : j + $ + W + O
          const F = e.elements.arrow && te(e.elements.arrow)
          const U = F ? (y === 'y' ? F.clientTop || 0 : F.clientLeft || 0) : 0
          ...
          const X = E[y] + q - V
          if (o) {
            const Y = oe(f ? ne(S, K) : S, D, f ? ie(N, X) : N)
            ;(E[y] = Y), (C[y] = Y - D)
          }
          if (a) {
            const Q = y === 'x' ? mt : bt
            ...
            const J = Z + g[Q]
            const tt = Z - g[G]
            const et = oe(f ? ne(J, K) : J, Z, f ? ie(tt, X) : tt)
            ;(E[w] = et), (C[w] = et - Z)
          }
        }
        e.modifiersData[n] = C
            ...
            if (Re(e, i)) {
              ;(a.rects = {
                ...
                a.orderedModifiers.forEach(
                  (t) => (a.modifiersData[t.name] = { ...t.data })
                )
  ...
  const $e = We()
  const ze = We({ defaultModifiers: [pe, Pe, ue, Ft] })
  const qe = We({ defaultModifiers: [pe, Pe, ue, Ft, Ie, xe, je, le, Ne] })
  ...
  class bi {
    ...
    _getConfig(t) {
      return (
        ((t = { ...pi, ...(typeof t === 'object' ? t : {}) }).rootElement = r(
          t.rootElement
        )),
        a('backdrop', t, mi),
        t
  ...
  const yi = { trapElement: 'element', autofocus: 'boolean' }
  ...
  j.on(
    ...
    g(Hi)
  const Bi = 'offcanvas'
  const Ri = { backdrop: !0, keyboard: !0, scroll: !1 }
  const Wi = { backdrop: 'boolean', keyboard: 'boolean', scroll: 'boolean' }
  const $i = 'show'
  ...
  class un extends B {
    ...
    _enter(t, e) {
      ;(e = this._initializeOnDelegatedTarget(t, e)),
        t && (e._activeTrigger[t.type === 'focusin' ? dn : hn] = !0),
    ...
    _leave(t, e) {
      ;(e = this._initializeOnDelegatedTarget(t, e)),
        t &&
          (e._activeTrigger[t.type === 'focusout' ? dn : hn] =
  ...
  const pn = { ...un.DefaultType, content: '(string|element|function)' }
  const mn = {
  ...
  g(gn)
  const _n = 'scrollspy'
  const bn = { offset: 10, method: 'auto', target: '' }
  const vn = { offset: 'number', method: 'string', target: '(string|element)' }
  const yn = 'active'
  ...
  const Nn = 'show'
  const In = 'showing'
  const Pn = { animation: 'boolean', autohide: 'boolean', delay: 'number' }
  const jn = { animation: !0, autohide: !0, delay: 5e3 }
  class Mn extends B {
    constructor(t, e) {
      super(t),
        (this._config = this._getConfig(e)),
