$(() => {
  ...
  $('#mainSidebarToggle').on('click', (e) => {
    e.preventDefault()
    $('body.horizontalmenu').toggleClass('main-navbar-show')
  })
  $('#mainContentLeftShow').on('click touch', (e) => {
    e.preventDefault()
    $('body').addClass('main-content-left-show')
  })
  $('#mainContentLeftHide').on('click touch', (e) => {
    e.preventDefault()
    $('body').removeClass('main-content-left-show')
  })
  $('#mainContentBodyHide').on('click touch', (e) => {
    e.preventDefault()
    $('body').removeClass('main-content-body-show')
  })
  $('body').append('<div class="main-navbar-backdrop"></div>')
  $('.main-navbar-backdrop').on('click touchstart', () => {
  ...
  $(document).on('click touchstart', (e) => {
    e.stopPropagation()
    const dropTarg = $(e.target).closest('.main-header .dropdown').length
    if (!dropTarg) {
      $('.main-header .dropdown').removeClass('show')
    }
    if (window.matchMedia('(min-width: 992px)').matches) {
      const navTarg = $(e.target).closest('.main-navbar .nav-item').length
      if (!navTarg) {
        $('.main-navbar .show').removeClass('show')
      }
      const menuTarg = $(e.target).closest('.main-header-menu .nav-item').length
      if (!menuTarg) {
      ...
      const hm = $(e.target).closest('.main-header-menu').length
      if (!hm) {
  ...
  $('.main-header-menu-header .close').on('click', (e) => {
    e.preventDefault()
    $('body').removeClass('main-header-menu-show')
  })
